    ...
    <v-table v-if="!loading" class="table custom" density="compact" fixed-header height="400px">
      <thead class="custom">
        <tr class="custom">
          ...
          <th class="custom" style="width: 22%">Actions</th>
        </tr>
      </thead>
      <tbody class="custom">
        <tr
...
const loading = workingTimeStore.isLoading
const error = workingTimeStore.error
const nextTempId = ref(0)
...
const deleteWorkingTime = async (idOrTempId) => {
  ...
  if (time.id) {
    await workingTimeStore.deleteWorkingTime(time.id)
  }
}
